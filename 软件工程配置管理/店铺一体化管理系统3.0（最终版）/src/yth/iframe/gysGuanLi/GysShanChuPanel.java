...
public class GysShanChuPanel extends JInternalFrame {// 供应商修改面板
	...
	public GysShanChuPanel() {// 供应商修改面板
		...
		delButton.addActionListener(new ActionListener() {// “删除”按钮动作事件的监听
			public void actionPerformed(ActionEvent e) {
				...
				if (item == null || !(item instanceof Item))// 数据表公共类对象为空或数据表公共类对象不是数据表公共类的实例
					return;// 退出程序
				int confirm = JOptionPane.showConfirmDialog(GysShanChuPanel.this, "确认删除供应商信息吗？");// 弹出“确认删除供应商信息吗？”提示框
				if (confirm == JOptionPane.YES_OPTION) {// 点击“确认”键
					...
					if (rs > 0) {// 删除供应商信息的数量大于0
						JOptionPane.showMessageDialog(GysShanChuPanel.this, "供应商：" + item.getName() + "。删除成功");// 弹出提示框
						gys.removeItem(item);// 移除“选择供应商”下拉列表中相匹配的数据表公共类对象
					} else {// 删除供应商信息的数量不大于0
						JOptionPane.showMessageDialog(GysShanChuPanel.this, "无法删除供应商：" + item.getName() + "。");// 弹出提示框
					}
				}
			}
		});
