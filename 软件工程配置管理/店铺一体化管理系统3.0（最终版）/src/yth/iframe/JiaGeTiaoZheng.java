...
public class JiaGeTiaoZheng extends JInternalFrame {// 价格调整内部窗体
	...
	public JiaGeTiaoZheng() {// 价格调整窗体的构造方法
		...
		setupComponet(new JLabel("单    位："), 2, 0, 1, 0, false);// 设置“单位”标签的位置并添加到容器中
		danwei = new JLabel();// “单位”标签
		danwei.setForeground(Color.BLUE);// 设置“规格”标签中的字体颜色为蓝色
		danwei.setPreferredSize(new Dimension(130, 21));// 设置“规格”标签的宽高
		setupComponet(danwei, 3, 0, 1, 1, true);// 设置“规格”标签的位置并添加到容器中
		...
		setupComponet(new JLabel("单　　价："), 2, 1, 1, 0, false);// 设置“单价”标签的位置并添加到容器中
		...
		setupComponet(danJia, 3, 1, 1, 100, true);// 设置“单价”文本框的位置并添加到容器中
		
		setupComponet(new JLabel("库存数量："), 0, 2, 1, 0, false);// 设置“库存数量”标签的位置并添加到容器中
		kuCunShuLiang = new JTextField();// “库存数量”文本框
		kuCunShuLiang.setEditable(false);// 设置“库存数量”文本框不可编辑
		setupComponet(kuCunShuLiang, 1, 2, 1, 1, true);// 设置“库存数量”文本框的位置并添加到容器中

		setupComponet(new JLabel("库存金额："), 2, 2, 1, 0, false);// 设置“库存金额”标签的位置并添加到容器中
		kuCunJinE = new JTextField();// “库存金额”文本框
		kuCunJinE.setEditable(false);// 设置“库存金额”文本框不可编辑
		setupComponet(kuCunJinE, 3, 2, 1, 100, true);// 设置“库存金额”文本框的位置并添加到容器中
		
		final JButton okButton = new JButton();// “确定”按钮
		okButton.addActionListener(new ActionListener() {// 为“确定”按钮添加动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				kcInfo.setSpdj(Double.valueOf(danJia.getText()));// 重置库存信息类中的单价
				kcInfo.setSpsl(Integer.valueOf(kuCunShuLiang.getText()));// 重置库存信息类中的库存数量
				int rs = Dao.updateKucunDj(kcInfo);// 获得修改库存的记录条数
				if (rs > 0)// 修改库存的记录条数大于0
					JOptionPane.showMessageDialog(getContentPane(), "价格调整完毕。", kcInfo.getName() + "价格调整",
		...
		shangPinMingCheng.addItemListener(new ItemListener() {// 为“商品名称”下拉列表添加选项事件的监听
			public void itemStateChanged(final ItemEvent e) {
				...
				dj = kcInfo.getSpdj().intValue();// 为“单价”赋值
				sl = kcInfo.getSpsl().intValue();// 为“库存数量”赋值
				chanDi.setText(kcInfo.getSpcd());// 设置“产地”标签里的文本内容
				danwei.setText(kcInfo.getSpdw());// 设置“单位”标签里的文本内容
				danJia.setText(kcInfo.getSpdj() + "");// 设置“单价”文本框里的文本内容
				kuCunShuLiang.setText(kcInfo.getSpsl() + "");// 设置“库存数量”文本框里的文本内容
