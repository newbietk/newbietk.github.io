...
public class ChuKuJiLu extends JInternalFrame {// 出库记录内部窗体
	
	private final JTable table;// 表格模型
	private final JTextField sjsj = new JTextField();// “出库时间”文本框
	private JComboBox jsr = null;// “经手人”下拉列表
	private final JComboBox gys = new JComboBox();// “供应商”下拉列表
	private final JTextField sjid = new JTextField();// “出库编号”文本框
	...
	private Date sjDate;// “出库时间”
	private JComboBox sp;// “商品”下拉列表

	public ChuKuJiLu() {// 进货-退货内部窗体的构造方法
		super();// 调用父类JInternalFrame的构造器
		setMaximizable(true);// 可以最大化
		setIconifiable(true);// 可以图标化
		setClosable(true);// 出库记录内部窗体可以关闭
		getContentPane().setLayout(new GridBagLayout());// 设置出库记录内部窗体的布局为网格布局
		setTitle("出库记录");// 设置出库记录内部窗体的标题
		setBounds(50, 50, 700, 400);// 设置出库编号内部窗体的位置和宽高
		// “出库编号”标签与“出库编号”文本框
		setupComponet(new JLabel("出库编号："), 0, 0, 1, 0, false);
		sjid.setFocusable(false);
		setupComponet(sjid, 1, 0, 1, 100, true);
		// “退货时间”标签与“退货时间”文本框
		setupComponet(new JLabel("出库时间："), 2, 0, 1, 0, false);
		...
		JButton rkButton = new JButton("出库");// “出库”按钮
		rkButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (jsrStr == null || jsrStr.isEmpty()) {
					JOptionPane.showMessageDialog(ChuKuJiLu.this, "请填写经手人");
					return;
				}
				if (ysjlStr == null || ysjlStr.isEmpty()) {
					JOptionPane.showMessageDialog(ChuKuJiLu.this, "填写验收结论");
					return;
				}
				if (table.getRowCount() <= 0) {
					JOptionPane.showMessageDialog(ChuKuJiLu.this, "添加出库商品");
					return;
				}
				...
				if (rs) {
					JOptionPane.showMessageDialog(ChuKuJiLu.this, "出库完成");
	...
	private void initsjid() {// 初始化“出库编号”
