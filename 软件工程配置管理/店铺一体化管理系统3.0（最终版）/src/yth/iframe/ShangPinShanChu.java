...
public class ShangPinShanChu extends JInternalFrame {// 商品管理内部窗体
	public ShangPinShanChu() {// 商品管理内部窗体的构造方法
		setIconifiable(true);// 可以图标化
		setClosable(true);// 可以关闭
		setTitle("商品管理");// 设置商品管理内部窗体的标题
		JTabbedPane tabPane = new JTabbedPane();// 创建选项卡面板
		final SpShanChu spscPanel = new SpShanChu();// 商品修改面板
		...
		tabPane.addTab("商品信息删除", null, spscPanel, "删除");// 把商品修改面板添加到选项卡面板中
		getContentPane().add(tabPane);// 把选项卡面板添加到商品管理内部窗体的内容面板中
		tabPane.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				spscPanel.initComboBox();// 初始化商品修改面板中的商品下拉选择框
				spscPanel.initGysBox();// 初始化商品修改面板中的供应商下拉列表
			}
		});
