...
public class ShangPingshuLiang extends JInternalFrame {// 库存盘点内部窗体
	...
	public ShangPingshuLiang() {// 库存盘点内部窗体的构造方法
	...
	private void initTable() {// 初始化表格
		String[] columnNames = { "商品名称", "商品编号", "供应商", "产地", "单位", "单价", "数量",  "盘点数量", "损益数量" };
		...
		for (int i = 0; i < kcInfos.size(); i++) {
			...
			if (spinfo.getId() != null && !spinfo.getId().isEmpty()) {
				row[0] = spinfo.getName();
				row[1] = spinfo.getId();
				row[2] = spinfo.getGysname();
				row[3] = spinfo.getSpcd();
				row[4] = spinfo.getSpdw();
				row[5] = info.get(2).toString();
				row[6] = info.get(3).toString();
				row[7] = 0;
				row[8] = 0;
