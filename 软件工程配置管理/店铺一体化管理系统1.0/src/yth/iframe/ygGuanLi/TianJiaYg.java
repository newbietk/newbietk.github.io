...
public class TianJiaYg extends JInternalFrame {// 添加经手人面板
	...
	public TianJiaYg() {// 添加经手人面板
		super();// 调用父类JPanel的构造器
		setMaximizable(true);
		setIconifiable(true);
		setClosable(true);
		getContentPane().setLayout(new GridBagLayout());// 设置添加经手人面板的布局为网格布局
		setTitle("添加员工");
		...
		add(ygName, gridBagConstraints_11);// 向添加经手人面板中添加“姓名”文本框
		...
		add(label_5, gridBagConstraints_12);// 向添加经手人面板中添加“编号”标签
		...
		add(bat, gridBagConstraints_15);// 向添加经手人面板中添加“银行账户”文本框
		...
		add(label_7, gridBagConstraints_16);// 向添加经手人面板中添加“性别”标签
		...
		add(status, gridBagConstraints_17);// 向添加经手人面板中添加“性别”下拉列表
		...
		button.addActionListener(new ActionListener() {// “添加”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				...
				TbZg user = Dao.getYg(nameStr, ageStr);// 经手人信息
				if (user.getSex() != null && !user.getSex().isEmpty()) {// 经手人的性别不为空且经手人表示性别的字符串的长度不为0
					JOptionPane.showMessageDialog(TianJiaYg.this, "经手人(" + user.getName() + ")已经存在");// 弹出对话框
					sex.setFocusable(true);// // “性别”下拉列表获得焦点
					return;// 退出应用程序
				}
				String sexStr = sex.getSelectedItem() + "";// 获得经手人的性别
				String statusStr = status.getSelectedItem() + "";
				TbZg yg = new TbZg();// 经手人信息
				yg.setTel(tel.getText());// 设置经手人电话
				yg.setAge(age.getText());// 设置经手人年龄
				yg.setName(nameStr);// 设置经手人姓名
				yg.setSex(sexStr);// 设置经手人性别
				yg.setId(id.getText());
				yg.setBat(bat.getText());
				yg.setFlag(statusStr);
				int i = Dao.addYg(yg);// 添加经手人
				if (i > 0)// 添加经手人的数目大于1
