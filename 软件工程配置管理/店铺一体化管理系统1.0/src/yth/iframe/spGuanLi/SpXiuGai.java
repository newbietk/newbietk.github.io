...
public class SpXiuGai extends JInternalFrame{
	private JComboBox sp;//“选择商品”下拉列表
	private JTextField mingCheng;//“商品名称”文本框
	private JTextField spid;//“商品编号”文本框
	...
	public SpXiuGai(){
		...
		setTitle("商品修改");//设置标题
		getContentPane().setLayout(new GridBagLayout());// 设置商品修改内部窗体的布局为网格布局
		...
		setupComponet(new JLabel("编号："), 0, 2, 1, 1, false);// 设置“编号”标签的位置并添加到容器中
		spid = new JTextField();// “编号”文本框
		setupComponet(spid, 1, 2, 3, 10, true);// 设置“编号”文本框的位置并添加到容器中
		...
		setupComponet(new JLabel("产地："), 0, 4, 1, 1, false);// 设置“产地”标签的位置并添加到容器中
		chanDi = new JTextField();// “产地”文本框
		setupComponet(chanDi, 1, 4, 3, 300, true);// 设置“产地”文本框的位置并添加到容器中

		setupComponet(new JLabel("供应商全称："), 0, 5, 1, 1, false);// 设置“供应商名称”标签的位置并添加到容器中
		gysMingCheng = new JComboBox();// “供应商名称”下拉列表
		gysMingCheng.setMaximumRowCount(5);// 设置“供应商名称”下拉列表显示的最大行数
		setupComponet(gysMingCheng, 1, 5, 3, 1, true);// 设置“供应商名称”下拉列表的位置并添加到容器中

		modifyButton = new JButton("修改");
		JPanel panel = new JPanel();
		panel.add(modifyButton);
		setupComponet(panel,1,6,1,0,false);
