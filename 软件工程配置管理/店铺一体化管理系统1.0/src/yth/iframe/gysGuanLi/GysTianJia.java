...
public class GysTianJia extends JInternalFrame {// 供应商添加面板
	...
	public GysTianJia() {// 供应商添加面板
		...
		tjButton.addActionListener(new ActionListener() {// “添加”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				// 文本框为空时，弹出提示框
				if (GysAddressF.getText().equals("") || GysNameF.getText().equals("") 
						|| GysMailF.getText().equals("")|| GysTelF.getText().equals("") 
						|| GysBatF.getText().equals("")|| GysTaxF.getText().equals("") ) {
					JOptionPane.showMessageDialog(GysTianJia.this, "请填写全部信息");
					return;
				}
				// 执行SQL查询语句获得的结果集
				ResultSet haveUser = Dao.query("select * from tbgysinfo where name='" + GysNameF.getText().trim() + "'");
				try {
					if (haveUser.next()) {// 结果集haveUser中有超过一条的记录
						System.out.println("error");// 控制台输出error
						JOptionPane.showMessageDialog(GysTianJia.this, "供应商信息添加失败，存在同名供应商", "供应商添加信息",
				...
				ResultSet set = Dao.query("select max(id) from tbgysinfo");// 执行SQL查询语句获得的结果集
				...
				JOptionPane.showMessageDialog(GysTianJia.this, "已成功添加客户", "客户添加信息",
						JOptionPane.INFORMATION_MESSAGE);// 弹出提示框
