...
public class ShangJiaJiLu extends JInternalFrame {// 上架记录内部窗体
	
	private final JTable table;// 表格模型
	private final JTextField sjsj = new JTextField();// “上架时间”文本框
	private JComboBox jsr = null;// “经手人”下拉列表
	private final JComboBox gys = new JComboBox();// “供应商”下拉列表
	private final JTextField sjid = new JTextField();// “上架编号”文本框
	private final JTextField spdj = new JTextField("0");// “商品单价”文本框
	private final JTextField spname = new JTextField("0");//"商品名称"文本框
	...
	private Date sjDate;// “上架时间”
	private JComboBox sp;// “商品”下拉列表

	public ShangJiaJiLu() {// 进货-退货内部窗体的构造方法
		super();// 调用父类JInternalFrame的构造器
		setMaximizable(true);// 可以最大化
		setIconifiable(true);// 可以图标化
		setClosable(true);// 上架记录内部窗体可以关闭
		getContentPane().setLayout(new GridBagLayout());// 设置上架记录内部窗体的布局为网格布局
		setTitle("上架记录");// 设置上架记录内部窗体的标题
		setBounds(50, 50, 700, 400);// 设置上架编号内部窗体的位置和宽高
		// “上架编号”标签与“上架编号”文本框
		setupComponet(new JLabel("上架编号："), 0, 0, 1, 0, false);
		sjid.setFocusable(false);
		setupComponet(sjid, 1, 0, 1, 100, true);
		// “退货时间”标签与“退货时间”文本框
		setupComponet(new JLabel("上架时间："), 2, 0, 1, 0, false);
	    sjsj.setFocusable(false);
		setupComponet(sjsj, 3, 0, 1, 120, true);
		// “供应商”标签与“供应商”下拉列表
		...
		JButton rkButton = new JButton("上架");// “上架”按钮
		rkButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String id = sjid.getText();// 票号
				String gysName = gys.getSelectedItem().toString();// 供应商名字
				if (jsrStr == null || jsrStr.isEmpty()) {
					JOptionPane.showMessageDialog(ShangJiaJiLu.this, "请填写经手人");
					return;
				}
				if (ysjlStr == null || ysjlStr.isEmpty()) {
					JOptionPane.showMessageDialog(ShangJiaJiLu.this, "填写验收结论");
					return;
				}
				if (table.getRowCount() <= 0) {
					JOptionPane.showMessageDialog(ShangJiaJiLu.this, "添加上架商品");
					return;
				}
				...
				for (int i = 0; i < rows; i++) {
					...
					detail.setTbsjMain(ShangJiaJiLu.getShangjiaId());
					detail.setDj(dj);
					detail.setSl(sl);
					set.add(detail);
				}
				...
				if (rs) {
					JOptionPane.showMessageDialog(ShangJiaJiLu.this, "上架完成");
	...
	private void initSpBox() {// 初始化“商品”下拉列表
		...
		ResultSet set = Dao.query("select * from tbkucun where " + "id in(select id from tbspinfo where gysName='"
				+ gys.getSelectedItem() + "')");
		sp.removeAllItems();
			...
			new Thread(new Runnable() {
				public void run() {
					try {
						while (true) {
							sjDate = new Date();
							sjsj.setText(sjDate.toLocaleString());
							Thread.sleep(1000);
						}
	...
	private void initsjid() {// 初始化“上架编号”
