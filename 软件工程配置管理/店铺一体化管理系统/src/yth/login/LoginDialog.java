...
public class LoginDialog extends JFrame{
	...
	private LoginPanel getLoginPanel(){//初始化loginPanel登陆面板的方法
		if(loginPanel == null){//登陆面板还没有组件
			...
			jLabel.setBounds(new Rectangle(85,41,76,18));//用户名标签的位置与宽高
			jLabel.setText("用户名：");//用户名标签文本内容
			jLabel1 = new JLabel();
			jLabel1.setBounds(new Rectangle(86,71,75,18));
			jLabel1.setText("密  码：");
	...
	private JPasswordField getPasswordField(){//初始化密码文本框
		if(passwordField == null){//如果密码文本框没有组件
			...
			passwordField.setBounds(new Rectangle(143,69,127,22));
	...
	private void initialize(){//初始化登陆窗体
		...
		setLocation((size.width-236)/2,(size.height-188)/2);//设置登陆窗体尺寸
		setSize(316,188);//设置登陆窗体的宽高
		this.setTitle("系统登陆");
