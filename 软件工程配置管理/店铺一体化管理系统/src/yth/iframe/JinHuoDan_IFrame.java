...
public class JinHuoDan_IFrame extends JInternalFrame {// 进货单内部窗体
	 ...
	 * @return “计算方式”文本框
	 */
	private JComboBox<String> getJsfsComboBox() {
		if (jsfsComboBox == null) {
			jsfsComboBox = new JComboBox<String>();
			jsfsComboBox.addItem("现金");
			jsfsComboBox.addItem("支付宝");
			jsfsComboBox.addItem("微信");
		}
		return jsfsComboBox;
	}
	...
	private JTable getTable() {
		if (table == null) {
			String[] columnNames = { "商品名称", "商品编号", "产地", "单位", 
					"单价", "数量" ,"供应商"};
			table = new JTable();
	...
	private JComboBox getSpComboBox() {
		if (spComboBox == null) {
			...
			spComboBox.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					ResultSet set = Dao
							.query("select * from tb_spinfo where name='"
	...
	private synchronized void updateTable() {
		...
		if (row >= 0 && spinfo != null) {
			...
			table.setValueAt(spinfo.getSpcd(), row, 2);
			table.setValueAt(spinfo.getSpdw(), row, 3);
			table.setValueAt(spinfo.getSpdj(), row, 4);
			table.setValueAt(spinfo.getSpsl(), row, 5);
