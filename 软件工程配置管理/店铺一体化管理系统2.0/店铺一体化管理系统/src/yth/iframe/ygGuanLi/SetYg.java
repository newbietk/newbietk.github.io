...
import yth.dao.Dao;
import yth.iframe.YgGL;

public class SetYg extends JPanel {// 员工设置窗体
	...
	public SetYg() {// 员工设置窗体
		setBounds(0, 0, 491, 200);// 设置“设置员工面板”的位置和宽高
		...
		final JButton enableButton1 = new JButton("禁用");// “禁用”按钮
		final JButton enableButton2 = new JButton("启用");//启用按钮
		final GridBagConstraints gridBagConstraints4 = new GridBagConstraints();// 创建网格限制对象
		final GridBagConstraints gridBagConstraints5 = new GridBagConstraints();// 创建网格限制对象
		gridBagConstraints4.insets = new Insets(5, 0, 5, 0);// 组件彼此的间距
		gridBagConstraints4.gridy = 7;// 组件位于网格的纵向索引为7
		gridBagConstraints4.gridx = 2;// 组件位于网格的横向索引为3
		gridBagConstraints5.insets = new Insets(5, 0, 5, 0);// 组件彼此的间距
		gridBagConstraints5.gridy = 7;// 组件位于网格的纵向索引为7
		gridBagConstraints5.gridx = 3;// 组件位于网格的横向索引为3
		add(enableButton1, gridBagConstraints4);// 向设置经手人面板中添加“禁用”按钮
		enableButton1.addActionListener(new ActionListener() {// “禁用/启用”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				int selRow = table.getSelectedRow();// 选定行的索引
				if (selRow < 0)// 选定行的索引小于0
					return;// 退出应用程序
				...
				if (op == JOptionPane.OK_OPTION) {// 点击“确认”
					...
					Dao.update("update tb_zg set flag=flag-1 where id='" + id + "'");// 执行SQL修改语句
					initTable();// 初始化表格模型
				}
			}
		});
		add(enableButton2, gridBagConstraints5);// 向设置经手人面板中添加“启用”按钮
		enableButton2.addActionListener(new ActionListener() {// “启用”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				int selRow = table.getSelectedRow();// 选定行的索引
				if (selRow < 0)// 选定行的索引小于0
					return;// 退出应用程序
		...
		button_1.addActionListener(new ActionListener() {// “关闭”按钮动作事件的监听
			public void actionPerformed(final ActionEvent e) {
				YgGL parent = (YgGL) SetYg.this.getRootPane().getParent();// 获得经手人设置窗体
